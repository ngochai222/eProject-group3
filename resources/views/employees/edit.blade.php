...
<div class="min-h-screen p-6">
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.employees.update', $employee->id) }}" method="POST" class="bg-gray-900 rounded-2xl p-6">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label class="block text-sm font-medium mb-2">Name</label>
                <input type="text" name="name" value="{{ $employee->name }}" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md" required>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium mb-2">Email</label>
                <input type="email" name="email" value="{{ $employee->email }}" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md" required>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium mb-2">Phone</label>
                <input type="text" name="phone" value="{{ $employee->phone }}" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md">
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="salary" value="{{ $employee->salary }}" step="0.01" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="room_id" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md">
                    ...
                    <option value="{{ $room->id }}" {{ $employee->room_id == $room->id ? 'selected' : '' }}>{{ $room->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="password" name="password" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md">
            </div>
